...
async def async_launch_persistent_context(
    playwright,
    ...
    **context_options,
    ...
    function = InjectFunction(fingerprint)
    # Build new context
    context = await playwright.chromium.launch_persistent_context(user_data_dir=profile, proxy={
        'server': proxy},
        headless=False,
    ...
    # Set headers
    await context.set_extra_http_headers(
        only_injectable_headers(fingerprint.headers, 'chrome')
    )
